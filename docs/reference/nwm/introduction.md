...
All **42 forecast configurations** published on `noaa-nwm-pds` are
curated (regenerated by `tools/nwm/refresh_nwm_catalog.py`) — `short_range`,
`medium_range` (members 1–6/7, CONUS + Alaska), `long_range` (members
1–4), the medium-range `blend`, the coastal total-water configs, the
regional Alaska / Hawaii / Puerto Rico variants, and the `forcing`
inputs. The `tmNN` analyses are excluded (different time axis). A few
representative rows:

| Configuration | Cycles (UTC) | Horizon | Note |
|---------------|--------------|---------|------|
| `short_range` | hourly (00–23) | 18 h | deterministic CONUS |
| `medium_range_mem1` | 00 / 06 / 12 / 18 | 240 h | GFS-forced ensemble member 1 |
| `long_range_mem1` | 00 / 06 / 12 / 18 | 720 h, 6-hourly | climate-range member 1 |
| `short_range_hawaii` | 00 / 12 | sub-hourly | 5-digit `fNNNNN` steps |

See the [Catalog & Tooling](catalog.md) page for the full breakdown.
