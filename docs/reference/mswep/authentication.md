...
| *"is an ensemble forecast stream … cannot address yet"* | The `Mid` / `Long` forecast streams are catalogued but not yet fetchable — a forecast granule is keyed by init time + member + lead. Use `Past` / `NRT`. |
