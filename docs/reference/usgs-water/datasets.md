...
The complete USGS parameter-code table is not hand-curated. The repository
includes an informational index, `available_parameters.yaml`, of **every** USGS
parameter code (~19,675 entries: code → name, group, unit), generated by
`tools/usgs_water/refresh_usgs_catalog.py` from the live USGS reference table
(`waterdata.get_reference_table(collection="parameter-codes")` — the modern
replacement for the removed `nwis.get_pmcodes`). It is a reference / tooling
artefact (used by the `validate` step and these docs) and is **not** loaded at
runtime nor shipped in the wheel — **any** valid code can be passed to
`variables=` directly regardless, via raw-code passthrough. Regenerate or
extend it with:
