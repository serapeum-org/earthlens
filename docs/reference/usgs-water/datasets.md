...
The complete USGS parameter-code table is not hand-curated. The bundled
`available_parameters.yaml` ships an informational index of **every** USGS
...
replacement for the removed `nwis.get_pmcodes`). Any of those codes can be
passed to `variables=` directly. Regenerate or extend it with:
