...
def _yaml_files_for(path: Path) -> list[Path]:
    ...
    Args:
        path: A directory of `*.yaml` files (the default layout) or a single
            `*.yaml` file (back-compat for tests that monkey-patch
            :data:`CATALOG_PATH`).
    ...
    if path.is_file():
        return [path]
    raise ValueError(
        f"Sentinel Hub catalog path {path} does not exist (expected a directory "
        "of *.yaml files, or a single YAML file)."
    )
