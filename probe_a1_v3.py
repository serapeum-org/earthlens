...
def main() -> None:
    ...
    print(f"  native block-wise : mean={np.nanmean(native_bw):.1f} "
    ...
    try:
        ...
        print(f"  native multi-block: mean={np.nanmean(native_mb):.1f} "
    ...
    for i in range(band.GetOverviewCount()):
        ...
        truth = native_bw[: oh * fh_, : ow * fw_].reshape(oh, fh_, ow, fw_).mean(axis=(1, 3))
        ...
        try:
            ...
            s1 = "RAISED" if False else (f"corr={c1:.4f} rel={r1:.3f}" if np.isfinite(c1) else "flat")
