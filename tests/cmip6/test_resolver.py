...
def test_resolve_explicit_version(resolver):
    ...
    out = resolver.resolve(
        source_id="CanESM5", experiment_id="ssp585", variable_id="tas",
        table_id="Amon", member_id="r1i1p1f1", version="20190101",
    )
...
def test_resolve_activity_and_grid_filters(resolver):
    ...
    out = resolver.resolve(
        source_id="GFDL-ESM4", experiment_id="ssp585", variable_id="pr",
        table_id="day", activity_id="ScenarioMIP", grid_label="gr1",
    )
    assert len(out) == 1
...
def test_resolve_miss_names_facet_and_values(resolver):
    ...
    with pytest.raises(ValueError, match="experiment_id='ssp999'"):
        resolver.resolve(
            source_id="CanESM5", experiment_id="ssp999", variable_id="tas", table_id="Amon",
        )
...
def test_resolve_miss_describes_prior_facets(resolver):
    ...
    with pytest.raises(ValueError, match="source_id=CanESM5"):
        resolver.resolve(
            source_id="CanESM5", experiment_id="ssp585", variable_id="tas", table_id="Emon",
        )
...
def test_resolve_explicit_version_miss(resolver):
    ...
    with pytest.raises(ValueError, match="available versions"):
        resolver.resolve(
            source_id="CanESM5", experiment_id="ssp585", variable_id="tas",
            table_id="Amon", member_id="r1i1p1f1", version="20200101",
        )
...
def test_slug_strips_path_separators():
    ...
    store = ResolvedStore(
        ...
        table_id="Amon", member_id="r1/i1", grid_label="gn", version="1",
    )
...
def test_select_version_no_version_column():
    ...
    out = resolver.resolve(
        source_id="M", experiment_id="e", variable_id="v", table_id="Amon",
    )
    assert len(out) == 1
