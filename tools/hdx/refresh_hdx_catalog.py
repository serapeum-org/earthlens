...
* `refresh` — run `Dataset.search_in_hdx(...)` filtered by organisation
  ...
  `catalog/_index.yaml`. This is **not** the full ~21k HDX catalogue —
...
def write_index(names: list[str], index_path: Path = INDEX_PATH) -> int:
    """Rewrite the `available_datasets:` index file, sorted and de-duped.

    Args:
        names: HDX dataset ids to record.
        index_path: Path to `catalog/_index.yaml`.
    ...
    header = (
        "# Informational index of HDX datasets discovered by the C7 refresh tool\n"
        "# (tools/hdx/refresh_hdx_catalog.py) for the curated organisations / "
        "themes.\n"
        "# Regenerate with: python tools/hdx/refresh_hdx_catalog.py refresh "
        "--org <org>\n"
    )
    body = yaml.safe_dump({"available_datasets": unique}, sort_keys=False)
...
def _cmd_refresh(args: argparse.Namespace) -> int:
    ...
    key is a member of the index, and rewrites `_index.yaml`.
    """
    configure()
