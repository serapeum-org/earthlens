...
requirement, so users can write `from earthlens import ...` for the
common cases:

* :class:`EarthLens` — the user-facing facade. Imports succeed without
...
`from earthlens.ecmwf import ECMWF` — or, more typically, through the
:class:`EarthLens` facade's `data_source=` argument.
"""
